...
import { NouvelleVenteEnginForm } from "./NouvelleVenteEnginForm";

export const maxDuration = 30;

export default async function NouvelleVenteEnginPage() {
  ...
  const [{ data: customers }, paymentMethods, vehicleModels] = await Promise.all([
    supabase.from("customers").select("id, name, phone, address, email").eq("business_id", user.businessId).order("name", { ascending: true }),
    getEnabledPaymentMethods(),
    getVehicleModelsAction(currentLocation.id),
  ]);

  return (
    <NouvelleVenteEnginForm
