...
import { ensureReferralCode, isReferralModuleEnabled, referralUrl, ZINDO_SITE, ZINDO_WHATSAPP } from "@/lib/referral";
import { isPurchaseOrdersModuleEnabled } from "@/lib/actions/purchase-orders";
import { isConfirmedOrder, purchaseOrderDisplayNumber, type PurchaseOrderStatus } from "@/lib/purchase-orders";
...
export default async function PurchaseOrderDocumentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [custom, referralOn] = await Promise.all([getInvoiceCustomization(user.businessId), isReferralModuleEnabled(user.businessId)]);
  const code = referralOn ? await ensureReferralCode(user.businessId) : null;
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2 print:hidden">
        <Link href={`/achats/commandes/${order.id}`} className="text-sm text-zinc-500 hover:underline">
          ← Retour
        </Link>
        <PrintDocumentButton />
      </div>
