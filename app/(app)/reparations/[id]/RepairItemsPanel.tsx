...
import { addRepairItemAction, removeRepairItemAction, type RepairTicketItemRow } from "@/lib/actions/repairs";
...
export function RepairItemsPanel({
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      const result = await addRepairItemAction(ticketId, formData);
      if (result?.error) {
  ...
  function handleRemove(itemId: string) {
    if (!confirm("Retirer cette pièce ? Le stock sera recrédité.")) return;
    setError(null);
    startTransition(async () => {
        ...
        <div className="space-y-3 rounded-xl border border-zinc-200 p-3">
          {product ? (
            ...
            <ProductPicker
              ...
              currency={currency}
              placeholder="Rechercher la pièce utilisée..."
              onSelect={(p) => {
                setProduct(p);
          ...
          <Button type="button" size="sm" disabled={!product || pending} onClick={handleAdd} className="w-full">
            <Plus className="h-3.5 w-3.5" /> {pending ? "Ajout..." : "Ajouter la pièce"}
          </Button>
        </div>
      )}
      ...
      {items.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHead>
              <tr>
                <TableHeaderCell>Pièce</TableHeaderCell>
