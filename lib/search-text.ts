...
function tokenMatches(queryToken: string, words: string[], haystack: string): boolean {
  if (haystack.includes(queryToken)) return true;
  // Fautes de frappe : seulement pour des mots assez longs, sinon trop de faux positifs.
...
/** Vrai si chaque mot de la recherche se retrouve (exactement ou presque) dans l'un des champs. */
export function matchesSearch(normalizedQuery: string, fields: (string | null | undefined)[]): boolean {
  if (!normalizedQuery) return true;
  const haystack = normalizeSearchText(fields.filter(Boolean).join(" "));
  const words = haystack.split(" ");
  return normalizedQuery.split(" ").every((t) => tokenMatches(t, words, haystack));
}
